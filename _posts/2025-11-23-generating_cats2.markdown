...
Another thing I tried is hierarchical LUTs, where the model generates coefficients for a mip chain of 8x8 patches that double in size - the coefficients are avg pooled from a 8x8 grid generated by the model. 64x8x8 dictionary is split into 4 groups: 8x8, 16x16, 32x32, 64x64 (upscaled from 8x8). The final image is composed by adding the patches from the different levels. This allows the model to capture both coarse and fine details independently.

![](/assets/gen_cats2/hiz1.png)

[Code](https://github.com/aschrein/pyd3d12/blob/master/tests/torch/cat_diffusion7.py)

Overall seems like quite a few approaches seem to work reasonably well for generating cat images.
